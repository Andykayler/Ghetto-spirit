"use client";

import Link from "next/link";
import { useEffect, useState } from "react";
import "./hero.css";

const socialSideLinks = [
  { handle: "@GHETTOSPIRIT_ENT", platform: "Instagram" },
  { handle: "@GHETTOSPIRIT_ENT", platform: "TikTok" },
  { handle: "GHETTO SPIRIT ENT", platform: "YouTube" },
  { handle: "GHETTO SPIRIT ENT", platform: "Facebook" },
  { handle: "@GHETTOSPIRITENT", platform: "X" },
];

const stats = [
  { num: "500+", label: "Artists Signed" },
  { num: "2M+",  label: "Monthly Streams" },
  { num: "150+", label: "Live Events" },
  { num: "12",   label: "Countries Reached" },
];

export default function HeroSection() {
  const [loaded, setLoaded] = useState(false);

  useEffect(() => {
    const t = setTimeout(() => setLoaded(true), 100);
    return () => clearTimeout(t);
  }, []);

  return (
    <section className="hero-section">
      {/* Background */}
      <div className={`hero-bg${loaded ? " loaded" : ""}`} />
      <div className="hero-vignette-left" />
      <div className="hero-vignette-tb" />

      {/* Main content row */}
      <div className="hero-content-row">

        {/* LEFT */}
        <div className={`hero-left${loaded ? " loaded" : ""}`}>

          {/* Eyebrow */}
          <div className="hero-eyebrow">
            <div className="hero-eyebrow-dot" />
            <div className="hero-eyebrow-line" />
            <span className="hero-eyebrow-text">Ghetto Spirit Entertainment</span>
          </div>

          {/* Headline */}
          <div className="hero-headline">
            <h1>FROM THE</h1>
            <h1>STREETS</h1>
            <h1 className="gold">TO THE STAGE</h1>
          </div>

          {/* Divider */}
          <div className="hero-divider" />

          {/* Sub */}
          <p className="hero-sub">
            RAW TALENT.{" "}
            <span className="accent">REAL CULTURE.</span>{" "}
            BIGGER DREAMS.
          </p>

          {/* Description */}
          <p className="hero-desc">
            Ghetto Spirit Entertainment is more than a brand.
            It&apos;s a movement built to discover, develop and promote raw
            talent from the ghetto to the world.
          </p>

          {/* CTAs */}
          <div className="hero-ctas">
            <Link href="/music" className="hero-btn-primary">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="black">
                <path d="M8 5v14l11-7z" />
              </svg>
              Listen Now
            </Link>
            <Link href="/artists" className="hero-btn-secondary">
              Explore Artists
            </Link>
          </div>
        </div>

        {/* Center spacer */}
        <div className="hero-center" />

        {/* RIGHT — social */}
        <div className={`hero-right${loaded ? " loaded" : ""}`}>
          <p className="hero-connect-label">Connect With Us</p>
          <div className="hero-social-list">
            {socialSideLinks.map((s) => (
              <a key={s.platform} href="#" className="hero-social-item">
                <span className="hero-social-icon">
                  <SocialIcon platform={s.platform} />
                </span>
                <span className="hero-social-handle">{s.handle}</span>
              </a>
            ))}
          </div>
        </div>
      </div>

      {/* Stats Bar */}
      <div className={`hero-stats-bar${loaded ? " loaded" : ""}`}>
        {stats.map((s) => (
          <div key={s.label} className="hero-stat">
            <div className="hero-stat-num">{s.num}</div>
            <div className="hero-stat-label">{s.label}</div>
          </div>
        ))}
      </div>
    </section>
  );
}

/* ── Icons ──────────────────────────────────────────────────── */
function SocialIcon({ platform }: { platform: string }) {
  if (platform === "Instagram") return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
    </svg>
  );
  if (platform === "TikTok") return (
    <svg width="12" height="14" viewBox="0 0 24 24" fill="currentColor">
      <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V9.02a8.16 8.16 0 0 0 4.77 1.52V7.1a4.85 4.85 0 0 1-1-.41z" />
    </svg>
  );
  if (platform === "YouTube") return (
    <svg width="16" height="12" viewBox="0 0 24 24" fill="currentColor">
      <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
    </svg>
  );
  if (platform === "Facebook") return (
    <svg width="8" height="14" viewBox="0 0 24 24" fill="currentColor">
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  );
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  );
}